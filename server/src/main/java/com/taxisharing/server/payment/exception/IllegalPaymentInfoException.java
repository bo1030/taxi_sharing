...
public class IllegalPaymentInfoException extends BaseException {
    public static final String ERROR_CODE_NOT_PAID = "payment-004";
    private static final String MESSAGE_NOT_PAID = "결제가 완료되지 않았습니다.";

    public static final String ERROR_CODE_INVALID_MERCHANT_UID = "payment-005";
    private static final String MESSAGE_INVALID_MERCHANT_UID = "결제 정보의 Merchant ID가 일치하지 않습니다.";

    public static final String ERROR_CODE_INVALID_AMOUNT = "payment-006";
    private static final String MESSAGE_INVALID_AMOUNT = "결제 정보의 Amount가 일치하지 않습니다.";

    public static final String ERROR_CODE_INVALID_CREATOR = "payment-007";
    private static final String MESSAGE_INVALID_CREATOR = "결제 정보의 PageName(상품정보)가 일치하지 않습니다.";

    public static final String ERROR_CODE_NOT_CANCELLED = "payment-008";
    ...
    public IllegalPaymentInfoException(String code, String message) {
        super(code, message);
    }

    public static IllegalPaymentInfoException from(String code) {
        if (ERROR_CODE_NOT_PAID.equals(code)) {
            return new IllegalPaymentInfoException(code, MESSAGE_NOT_PAID);
        }

        if (ERROR_CODE_INVALID_MERCHANT_UID.equals(code)) {
            return new IllegalPaymentInfoException(code, MESSAGE_INVALID_MERCHANT_UID);
        }

        if (ERROR_CODE_INVALID_AMOUNT.equals(code)) {
            return new IllegalPaymentInfoException(code, MESSAGE_INVALID_AMOUNT);
        }

        if (ERROR_CODE_INVALID_CREATOR.equals(code)) {
            return new IllegalPaymentInfoException(code, MESSAGE_INVALID_CREATOR);
        }

        if (ERROR_CODE_NOT_CANCELLED.equals(code)) {
            return new IllegalPaymentInfoException(code, MESSAGE_NOT_CANCELLED);
        }
